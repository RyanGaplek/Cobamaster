...
API_KEY = "mr_live_q49JtrVtsQU83nxkYCKc0nXfgwKGfkgc"
BOT_NAME = "peaxel13"                
...
NAMA_SCRIPT = os.path.basename(__file__).replace(".py", "")
SESSION_FILE = f"session_{NAMA_SCRIPT}.json"
...
def register_agent(game_id):
    ...
    try:
        ...
        print(f"⚠️ [{BOT_NAME}] Error koneksi saat daftar: {e}")
        return None
...
def get_weapon_score(weapon_name):
    ...
    if "fist" in name_lower or "none" in name_lower: 
        return 0
    if "sniper" in name_lower or "rifle" in name_lower: 
        return 60
    if "katana" in name_lower: 
        return 50
    if "pistol" in name_lower or "gun" in name_lower: 
        return 40
    if "sword" in name_lower: 
        return 30
    if "bow" in name_lower: 
        return 20
    if "knife" in name_lower or "dagger" in name_lower: 
        return 10
    return 5 
...
def sort_loot_priority(item_data):
    ...
    # 🔥 URUTAN KASTA LOOTING TERBARU 🔥
    if "moltz" in nl or "coin" in nl: 
        return 999  # UANG ADALAH DEWA! PRIORITAS MUTLAK!
    if "medkit" in nl or "bandage" in nl or "emergency" in nl: 
        return 100 
    if "sniper" in nl or "katana" in nl or "rifle" in nl: 
        return 80 
    if "ration" in nl or "potion" in nl: 
        return 50
    return 10
...
def cari_pintu_strategis(pintu_aman, region_dict, hp_sekarat):
    ...
    if hp_sekarat and len(forests) > 0: 
        return random.choice(forests)
    if not hp_sekarat and len(ruins) > 0: 
        return random.choice(ruins)
...
def decide_action(state, bot_memory):
    ...
    try:
        my_hp_val = int(self_data.get("hp", 100))
    except:
        my_hp_val = 100
    ...
    for r in adjacent_regions:
        r_str = str(r.get("id") if isinstance(r, dict) else r).lower()
        adjacent_ids.append(r_str)
    ...
    # Setup Memori yang dibutuhkan
    if "dz_memory" not in bot_memory: 
        bot_memory["dz_memory"] = set()
    if "pdz_memory" not in bot_memory: 
        bot_memory["pdz_memory"] = set()
    if "sampah_memory" not in bot_memory: 
        bot_memory["sampah_memory"] = set()
    ...
    for pdz in raw_pdz:
        if isinstance(pdz, dict): 
            bot_memory["pdz_memory"].add(str(pdz.get("id", "")).lower())
        else: 
            bot_memory["pdz_memory"].add(str(pdz).lower())

    for dz in raw_dz:
        if isinstance(dz, dict): 
            bot_memory["dz_memory"].add(str(dz.get("id", "")).lower())
        else: 
    ...
    is_death_zone_now = False
    if current_r_id in bot_memory["dz_memory"] or current_r_name in bot_memory["dz_memory"] or region.get("isDeathZone", False):
        is_death_zone_now = True

    is_pending_dz_now = False
    if current_r_id in bot_memory["pdz_memory"] or current_r_name in bot_memory["pdz_memory"] or region.get("isPendingDeathZone", False):
        is_pending_dz_now = True
    ...
    for fac in interactables:
        if not fac.get("isUsed"):  
            ...
            fac_id = fac.get("id")
            if "medical" in fac_name: 
                id_medical = fac_id
            elif "supply" in fac_name: 
                id_supply = fac_id
    ...
    if equipped_item:
        ...
        if "fist" in nm_low or "none" in nm_low:
            ...
            if any(w in nm_low for w in ["bow", "pistol", "sniper", "rifle", "gun"]): 
                weapon_range = 1
    ...
    for item in inventory:
        ...
        if "megaphone" in name_lower: 
            punya_megaphone = True
        ...
        if is_equipped:
            if "fist" in name_lower or "none" in name_lower:
                ...
                if any(w in name_lower for w in ["bow", "pistol", "sniper", "rifle", "gun"]): 
                    weapon_range = 1
            continue
    ...
    # 🔥 IDENTIFIKASI MUSUH vs SAUDARA PEAXEL 🔥
    musuh_player = []
    musuh_monster = []
    teman_player = []
    ...
    for a in semua_orang:
        if a.get("isAlive", True) and a.get("id") != my_id:
            ...
            jarak_ke_musuh = 99
            if m_reg_id == current_r_id:
                jarak_ke_musuh = 0
            elif m_reg_id in adjacent_ids:
                jarak_ke_musuh = 1
            ...
            if jarak_ke_musuh <= max(weapon_range, 1):
                ...
                is_monster = False
                if "type" in a and a["type"] in ["monster", "npc"]: 
                    is_monster = True
                if any(m in str(a.get("name", "")).lower() for m in ["wolf", "bear", "bandit"]): 
                    is_monster = True
                
                # Filter IFF (Keluarga Peaxel)
                if "peaxel" in str(a.get("name", "")).lower():
    ...
    if len(musuh_player) > 0:
        musuh_player_terlemah = musuh_player[0]
    else:
        musuh_player_terlemah = None
        
    if len(musuh_monster) > 0:
        musuh_monster_terlemah = musuh_monster[0]
    else:
        musuh_monster_terlemah = None
        
    musuh_player_sekamar = []
    for m in musuh_player:
        if m.get("jarak") == 0:
            musuh_player_sekamar.append(m)
            
    jumlah_pengeroyok = len(musuh_player_sekamar)
    
    teman_sekamar = []
    for t in teman_player:
        if t.get("jarak") == 0:
            teman_sekamar.append(t)
            
    jumlah_teman = len(teman_sekamar)
    kekuatan_tim = 1 + jumlah_teman # 1 itu bot ini sendiri
    ...
    def aksi_move(pesan_kustom="🚪 Melipir cari aman..."):
        ...
        for r in adjacent_regions:
            ...
            if r_id in bot_memory["dz_memory"] or r_obj.get("isDeathZone") or r_obj.get("isDeathzone"): 
                is_dz = True
            if r_id in bot_memory["pdz_memory"] or r_obj.get("isPendingDeathZone") or r_obj.get("isPendingDeathzone"): 
                is_pdz = True
        ...
        if len(pintu_aman) > 0:
            pilihan_baru = []
            for r in pintu_aman:
                if r not in bot_memory["visited_path"]:
                    pilihan_baru.append(r)
            ...
            if len(pilihan_baru) > 0: 
                ...
                pilihan_darurat = []
                for r in pintu_aman:
                    if r != ruangan_sebelumnya:
                        pilihan_darurat.append(r)
                        
                if len(pilihan_darurat) > 0:
                    target_id = cari_pintu_strategis(pilihan_darurat, region_dict, my_hp_val < 60)
                else:
                    target_id = cari_pintu_strategis(pintu_aman, region_dict, my_hp_val < 60)
        ...
        if target_id:
            ...
            if len(bot_memory["visited_path"]) > 10: 
    ...
    def aksi_equip(item_id): 
        return {"type": "equip", "itemId": item_id}
    ...
    # ================== PUNGUT BARANG (MATA DUITAN MODE) ==================
    if len(barang_di_area) > 0:
        for item_terbaik in barang_di_area:
            ...
            if "megaphone" in nm_low and punya_megaphone: 
                continue
            if "radio" in nm_low and punya_radio: 
                continue
            if "map" in nm_low and punya_map: 
                continue
            ...
            is_koin = False
            if "moltz" in nm_low or "coin" in nm_low:
                is_koin = True
            
            # 🔥 KALAU ADA KOIN, SIKAT APAPUN YANG TERJADI! 🔥
            if is_koin:
                smart_print(bot_memory, f"[{BOT_NAME}] 💰 MATA DUITAN! Ada {nama_barang}, SIKAT!")
                return aksi_pungut(item_terbaik)
            
            # Kalau tas belum penuh, pungut barang berguna lainnya (Medkit dll)
            if not tas_penuh:
    ...
    # 🔥 PROTOKOL KARTEL PEAXEL (KALKULATOR KEROYOKAN) 🔥
    # Kalau musuh jauh lebih banyak dari total kekuatan geng kita, kabur!
    if jumlah_pengeroyok >= 3 and jumlah_pengeroyok > kekuatan_tim:
        ...
        if aksi: 
            ...
            smart_print(bot_memory, f"[{BOT_NAME}] 🛑 ZONA AKHIR BUNTU! Gak bisa kabur, terpaksa TAWURAN SINI KAU!")
    ...
    # 🔥 SMART COMBAT LOGIC (NINJA ASSASSIN & GANG BRAWL) 🔥
    if musuh_player_terlemah:
        ...
        # 1. Kalau lagi tangan kosong, jangan sok jago
        if tangan_kosong:
        ...
        # 2. Kalau ada Mafia Peaxel (Saudara) di ruangan, BANTUIN GABUK!
        if jarak_musuh == 0 and jumlah_teman > 0:
            smart_print(bot_memory, f"[{BOT_NAME}] 🤝 GANKING MAFIA! Bantu saudara hajar {nama_musuh}!")
            return aksi_serang(target.get("id"), "agent")

        # 3. KALKULATOR OPORTUNIS (Biar gak mati konyol)
        if jarak_musuh == 0:
            # HANYA serang kalau musuh HP-nya merah (<40) ATAU darah kita penuh banget (>85) ATAU hp musuh di bawah kita
            if hp_musuh <= 40:
                ...
                # Musuh masih sehat dan darah kita pas-pasan? MUNDUR! Biar dia gelut sama orang lain.
        ...
        # 4. Musuh di ruangan sebelah (Sniper)
        elif jarak_musuh > 0:
            if weapon_range > 0:
                smart_print(bot_memory, f"[{BOT_NAME}] 🎯 SNIPER! Tembak {nama_musuh} dari jauh!")
                return aksi_serang(target.get("id"), "agent")
            else:
                # Cuma ngejar kalau musuh beneran sekarat banget (<30)
    ...
    if musuh_monster_terlemah:
        ...
        # Cuma mau buang-buang waktu gebuk monster kalau darah di atas 80!
...
def main():
    if API_KEY.startswith("ISI_"): 
        fatal("API KEY belum diisi! Ganti di bagian atas script.")
    ...
    if not resume_berhasil:
        ...
        while not agent_id:
            ...
            if not game_id:
                print(f"🔄 [{BOT_NAME}] Re-scan radar dalam 1.5 detik...\n")
                time.sleep(1.5)
                continue
            ...
            if not agent_id:
                print(f"⏩ [{BOT_NAME}] Keduluan! Langsung tembak room lain...\n")
                time.sleep(0.5) 
